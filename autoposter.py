...
def log(msg: str):
    """Schrijft logregels zowel naar console als naar lokaal logbestand."""
    now = datetime.now(timezone.utc).strftime("[%H:%M:%S]")
    entry = f"{now} {msg}"
    print(entry)
    with open("autoposter_log.txt", "a", encoding="utf-8") as f:
        f.write(entry + "\n")
...
def main():
    ...
    log("📁 Log opgeslagen in autoposter_log.txt")
